...
class Table:
    def __init__(self, number):
        self.number = number
        self.guest = None  # Стол свободен, гость - None
...
class Guest(Thread):
    ...
    def run(self):
        # Гость задерживается от 3 до 10 секунд
        ...
        for guest in guests:
            ...
            for table in self.tables:
                if table.guest is None:  # Стол свободен
                    ...
                    seated = True
                    break
            if not seated:  # Если мест нет, помещаем в очередь
            ...
            for table in self.tables:
                if table.guest is not None and not table.guest.is_alive():
                    ...
                    if not self.queue.empty():
                        next_guest = self.queue.get()  # Берем из очереди
